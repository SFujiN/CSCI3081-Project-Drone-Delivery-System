...
std::unordered_map<std::string, csci3081::droneSpecs>
csci3081::createDroneModelList(std::string filename) {
...
/* NOTICE: Commented out because passes by non-const reference
void csci3081::updateDroneModelList(
    std::unordered_map<std::string, csci3081::droneSpecs>& list,
    std::string filename) {
  std::fstream fin;
  std::string line, word;
  std::unordered_map<std::string, int> header;
